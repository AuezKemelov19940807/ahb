...
const stateItems = useItemsStore()
...
const update = () => {
  storeItems.updateItem({ ...item.value })
...
onMounted(() => {
  if (id) {
    const fetchedItem = storeItems.getUserById(id)
    item.value = { ...fetchedItem }
  }
})
</script>
<template>
  <div v-if="stateItems.isLoading">loading...</div>
  <form v-else @submit.prevent="performAction">
